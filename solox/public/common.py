...
class Devices:
    ...
    def getSDKVersion(self, deviceId):
        """获取设备的sdk版本"""
        command = 'adb -s ' + deviceId + " shell getprop ro.build.version.sdk"
        result = os.popen(command)
        sdk_version = int(result.read().strip())
        return sdk_version

    def getPid(self, deviceId, pkgName):
        """获取对应包名的pid"""
        if self.getSDKVersion(deviceId) > 25:
            # Android8以后，sdk版本大于25要用ps -A
            if platform.system() != 'Windows':
                result = os.popen(f"adb -s {deviceId} shell ps -A | grep {pkgName}").readlines()
            else:
                result = os.popen(f"adb -s {deviceId} shell ps -A | findstr {pkgName}").readlines()
        else:
            if platform.system() != 'Windows':
